...
func Run(opts RunOptions) error {
	...
	if firstRun {
		...
		fmt.Println(dim.Padding(0, 2).Render("  clawvisor-server services    — connect GitHub, Gmail, Slack, etc."))
		fmt.Println(dim.Padding(0, 2).Render("  clawvisor-server connect-agent — connect Claude Code, Claude Desktop, etc."))
		fmt.Println()
	}
...
// overwrite it. Use `clawvisor-server services` and `clawvisor-server connect-agent` to
...
func Setup() error {
	...
	fmt.Println(dim.Padding(0, 2).Render("  clawvisor-server install     — install and start the daemon"))
