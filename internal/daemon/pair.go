	...
	for {
		...
		case <-ticker.C:
			if time.Now().After(deadline) {
				return fmt.Errorf("pairing timed out — try again with: clawvisor-server pair")
			}
